...
* [手动安装](#手动安装)
    * [普通版本](#普通版本)
        ...
        * [安装LXD](#安装LXD)
        * [多次批量生成LXC虚拟化的容器](#多次批量生成LXC虚拟化的容器)
    * [纯探针版本](#纯探针版本)
    * [查看log文件](#查看log文件)
    * [注意事项](#注意事项)
...
开出的容器配置：1核256MB内存1GB硬盘限速250Mbps带宽
...
内存看你开多少容器，这里如果要开8个，换算需要2G内存，实际内存如果是512MB内存，还需要开1.5G，保守点开2G虚拟内存即可
...
```bash
! incus -h >/dev/null 2>&1 && echo 'alias lxc="/snap/bin/lxc"' >> /root/.bashrc && source /root/.bashrc
...
lxc若命令无问题，执行初始化开容器，这一步最好放```screen```中后台挂起执行，开容器时长与你开几个和母鸡配置相关

执行下面命令加载开机脚本

```
rm -rf init.sh
wget https://github.com/oneclickvirt/incus/raw/main/scripts/init.sh
chmod 777 init.sh
...
开出的容器配置：1核128MB内存300MB硬盘限速200Mbps带宽
...
内存看你开多少容器，这里如果要开10个，换算需要1G内存，实际内存如果是512MB内存，还需要开0.5G，保守点开1G虚拟内存即可
...
lxc命令无问题，执行初始化开容器，这一步最好放screen中后台挂起执行，开容器时长与你开几个和母鸡配置相关

加载开机脚本

```
rm -rf least.sh
wget https://github.com/oneclickvirt/incus/raw/main/scripts/least.sh
chmod 777 least.sh
apt install dos2unix -y
dos2unix least.sh
```
...
#### 不要拿该脚本开出的容器当生产环境，lxc虚拟化不支持换内核，dd，开启bbr，**纯探针版本**(普通版本无问题)挂载warp等操作
<details>
...
```
incus list | awk '{print $2}' | grep -v "^$" | xargs -I {} incus delete -f {}
```
...
这里的虚拟内存是说要开的SWAP大小，存储池则是你所有要开的容器占的盘的大小的总和
...
支持自定义容器的系统，注意传入参数为系统名字+版本号，如：debian11、ubuntu20，centos7，注意都是小写字母+数字的组合

```
./buildone.sh 容器名称 内存大小 硬盘大小 SSH端口 外网起端口 外网止端口 下载速度 上传速度 是否启用IPV6(Y or N) 系统(留空则为debian11)
```
...
这样就是创建一个名为test的容器，内存256MB，硬盘2G，SSH端口20001，内外网起止端口20002~20025，下载和上传速度都设置为300Mbit，且不自动设置外网IPV6地址，留空系统默认使用debian11

如果已通过以上方法生成过容器，还需要批量生成容器，可使用手动安装部分的脚本
...
```
curl -L https://github.com/oneclickvirt/incus/raw/main/scripts/rules.sh -o rules.sh && chmod +x rules.sh && bash rules.sh
```
...
```
curl -L https://github.com/oneclickvirt/incus/raw/main/scripts/build_monitor.sh -o build_monitor.sh && chmod +x build_monitor.sh && bash build_monitor.sh
```
